...
namespace StoreWin
{
    public partial class PurchasesRet : Form
    {
        public PurchasesRet()
        {
            InitializeComponent();
        ...
        private void PurchasesRet_Load(object sender, EventArgs e)
        {
            Bindinvno();
        }
                ...
                else
                {
                    if (txt_qty.Value > 0)
                    {
                        ...
                        else
                        {
                            ...
                            string sSQL = "INSERT INTO retpurchases(product_id,qty,purchases_id)";
                            sSQL += "Values(@product_id,@qty,@purchases_id);";
                            dbCommand.CommandText = sSQL;

                            dbCommand.Parameters.AddWithValue("@product_id", prodid);
                            dbCommand.Parameters.AddWithValue("@qty", txt_qty.Text);
                            dbCommand.Parameters.AddWithValue("@purchases_id", combo_invno.SelectedValue);
